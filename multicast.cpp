...
int if_NameToIndex(std::string ifname, char *address)
{
    ...
    printf("Using device %s : Agent IP -> %s\n", if_info.ifr_name, address);
...
//  Open Socket
//
int Multicast::openSocket(std::string interface, int recv_own_data)
{
   ...
   multicastAddress.sin_addr.s_addr = INADDR_ANY;
    ...
    {
        printf("Error creating Multicast UDP socket : %s",strerror(errno));
        return -1;
    }
    ...
    {
       printf("Error setting socket option 1: %s",strerror(errno));
        return -1;
    }
    ...
    {
      printf("Error setting socket option 2: %s",strerror(errno));
        return -1;
    }
    ...
    {
       printf("Error setting socket option 3: %s",strerror(errno));
        return -1;
    }
    ...
    {
        printf("Error setting socket option 4: %s",strerror(errno));
        return -1;
    }
...
//  Send Data
//
int Multicast::sendData(void* data, int dataSize)
{
...
//  Receive Data
//
int Multicast::receiveData(void* buffer, int bufferSize)
{
