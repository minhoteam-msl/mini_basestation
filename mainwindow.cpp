    ...
    ui(new Ui::MainWindow)
{
    ...
    for(int i=0;i<numberRobots;i++)
    {
        ...
        #endif
        #ifdef lar
        ball[i]->setPos(getx(-1.5)+((i-4)*(ROBOT_DIAMETER*1.5/FACTOR)),gety(4));
        #endif
...
void MainWindow::change_robot_info(minho_team_ros::interAgentInfo incoming_data)
{
    ...
    setRobotLablePosition(incoming_data.agent_info.robot_info.robot_pose.x,incoming_data.agent_info.robot_info.robot_pose.y,incoming_data.agent_id);
    setRobotLableAngle(incoming_data.agent_info.robot_info.robot_pose.z,incoming_data.agent_id);
...
void MainWindow::setRobotLablePosition(float x, float y, int robot)
{
    switch (robot) {
    case 1:
    ui->lb_pos_r1->setText("x:"+QString::number(getMeterx(x))+";y:"+QString::number(getMetery(y)));
        break;
    case 2:
    ui->lb_pos_r2->setText("x:"+QString::number(getMeterx(x))+";y:"+QString::number(getMetery(y)));
        break;
    case 3:
    ui->lb_pos_r3->setText("x:"+QString::number(getMeterx(x))+";y:"+QString::number(getMetery(y)));
        break;
    case 4:
    ui->lb_pos_r4->setText("x:"+QString::number(getMeterx(x))+";y:"+QString::number(getMetery(y)));
        break;
    case 5:
    ui->lb_pos_r5->setText("x:"+QString::number(getMeterx(x))+";y:"+QString::number(getMetery(y)));
        break;
    default:
        break;
    }
